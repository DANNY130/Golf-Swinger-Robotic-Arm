...
void IK::inverseKinematics(double x, double y, double z, double t[3][3], double angles[6], bool flip)
{
    ...
    angles[0] = atan2(yc,xc);

    double D = ( pow(xc,2) + pow(yc,2) + pow((zc-m_IK.d1),2) - pow(m_IK.a2,2) - pow(m_IK.d4,2) ) / (2*m_IK.a2*m_IK.d4);
    angles[2] = atan2(-sqrt(1 - pow(D,2)), D);

    double k1 = m_IK.a2+m_IK.d4*cos(angles[2]);
    double k2 =         m_IK.d4*sin(angles[2]);
    angles[1] = atan2( (zc-m_IK.d1), sqrt(pow(xc,2) + pow(yc,2)) ) - atan2(k2,k1) ;
    ...
    double ax =  r13*cos(q1)*cos(q2 + q3) + r23*cos(q2 + q3)*sin(q1) + r33*sin(q2 + q3);
    double ay = -r23*cos(q1)      + r13*sin(q1);
    double az = -r33*cos(q2 + q3) + r13*cos(q1)*sin(q2 + q3) + r23*sin(q1)*sin(q2 + q3);
    double sz = -r32*cos(q2 + q3) + r12*cos(q1)*sin(q2 + q3) + r22*sin(q1)*sin(q2 + q3);
    double nz = -r31*cos(q2 + q3) + r11*cos(q1)*sin(q2 + q3) + r21*sin(q1)*sin(q2 + q3);
